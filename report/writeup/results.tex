...
\sisitodo[inline]{Can you read this section again, have a look at what I changed -- that is pretty much most of the paragraphs in the end as I wanted to incorporate the different ways of looking into the data that became apparent when writing the section above.}
\sisitodo[inline]{Can you also look into the figures and ensure they are properly referenced, I kind of wanted to do this fast and did not include half the autorefs I should}
\sisitodo[inline]{Finally the figure alignments are off, as you noticed yourself, try playing with "scale=", rather than "width=" parameter for the square-ish figures, and see if that would help. If not leave this for me, I will fix them later on.}
...
(\autoref{fig:inference_phenomenon:higher_order}). This is completely unexpected, as we are inclined to believe that higher-order moments are necessary in order to capture the he damped oscillatory behaviour of the system. The existence of such lower-distance area shows that minima points exist that are easy enough to approximate so the higher order information is not required, and that can produce trajectories very close to the observed data, even though their parameter values are different.
...
This does not seem to be the case with parameter inference, as we can see the distance landscape changing completely between the figures \autoref{fig:inference_phenomenon:maxorder:2:landscape} and \autoref{fig:incerence_phenomenon:maxorder:5:landscape}.
...
From the data in hand, we cannot say anything about the other low-distance area resulting for very large values of the parameter $c_6$, as seen in \autoref{fig:inference_phenomenon:maxorder:1:landscape}, as this parameter range has not been explored by higher-order inference procedures. We believe that it was the large starting distance from the observed trajectories when lower order \gls{mea} approximations are used  (see the red line in \autoref{fig:parameter_pair_c2_c6} marking the starting trajectory), that created a large gradient allowing the inference procedure to explore a larger parameter space. The slope for higher order moment approximations has been considerably lower, and therefore less parameter space has been explored.

Generally, the results described in this section seem to point that one must be very careful when relying on \gls{mea} approximations when doing parameter inference. Of course, the approximations and by extension parameter inference become slightly better as maximal order increases, and at least some of the low-distance easy-to-simulate islands disappear as higher order moment expressions are incorporated. Nevertheless, we were not able to recover the original parameter set in any of the experiments attempted. This raises a question: if we were not able to recover the starting set of parameters in this toy-example, how can we recover them in an unknown biological system? 
...
From the data in hand, we could infer that relative change in the \verb"MDM2" degradation rate impacts the system less than a change in $c_2$ parameter.

In conclusion, the results in this section raise the concerns of validity of single-point estimates obtained through the parameter inference procedure. As shown in \todo{which figurefigure?} were able to get a near-perfect match between the two trajectories, with sets of parameters that were nowhere close to the ones used for the data generation. 
...
\begin{figure}
    ...
    \caption{}
    \label{fig:inference_phenomenon:maxorder:5:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[b]{0.7\textwidth}
