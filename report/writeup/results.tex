...
In addition, ...\\
\todo{@SL short intro about why it is important to make solver fast}
...
In order to study the performance of the inference method in \means package, we used "sum of squares" distance method and p53 model. Due to the lack of experimental concentrations of the species, the averaged concentration change through time from $5000$ \gls{gssa} are used as the observed trajectories. With full variability for all parameters in the p53 model, and starting values equal to the values from \todo{Ref}, it is expected that the resulting parameter values would be the same as the starting values. To visualise how the system explored the multi-dimensional parameter space, we studied the cross-section for all possible pairs of parameters and the trajectory of how concentration for each species changed through time. Unexpectedly, trajectories simulated with the true parameter values are identified as distant from the \gls{gssa} trajectories, which forced the system to follow the distant gradient and infer incorrect parameter values. This result can be explained by inference method trapped at local minima, but not the global minima.

To test if the unexpected inference results do not lie within the logbal minima, we re-generated the distance landscape using the end points of the minima previously identified as the starting values (see Figure \todo{figure number}). Here, we explicitly allow $c_2$ and $c_6$ values to vary, but keep other parameter values the true values, because $c_2$ and $c_6$ with variability in their values are able to produce an almost perfect match with the \gls{gssa} trajectories of all species (data not shown). Using the end point of the local minima as the starting point for $c_2$ and $c_6$, the resulting distance landscape figures and trajectories for each species clearly indicate that the previously identified local minima are not the global minima, as the system again travelled across a wide parameter space. 

The distance landscape figures also shows that although the distance gradient becomes shallower as the maximal order for \gls{mea} increases, the inferred parameter values for $c_6$ are mostly more than 10 times larger than the true value, despite the starting value. 
