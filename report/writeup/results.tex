...
\begin{figure}
    ...
    \caption{\emph{Contour plots of the sum-of-squares distance between     the \gls{mea} approximated trajectory and the \gls{gssa} simulated     trajectory for scalar closure and \texttt{ode15s} solver}. The grid of distances was calculated by sampling each of the two parameters $c_2$ and $c_4$ at $0.1$ intervals and comparing the simulated trajectoreis to the \gls{gssa} trajectory. Areas where \gls{ode} solver has failed are marked by black-and-white stripes. 
    The four points indicated with $\bullet$, $\ast$, $\times$ and $\blacksquare$ are the points of interest whose behaviour is studied in other figures. 
    The set of parameters used in \autoref{fig:max_order_and_closure_on_distance_summary} is indicated by a $\star$. 
    Note that as the maximum order parameter increases, so does the relatively low-distance area. Unfortunately, the area in which the system is stable shrinks at a similar rate. 
    It is also evident that even moment orders seem to be more stable than the odd ones.}
    
    \label{fig:hit-and-miss:ode15s}
\end{figure}

The previous section has described that it may be hard to predict which maximum order to use in order to get best approximation of the true behaviour of the system \emph{a priori}. 
...
In order to achieve this, we first fixed all the parameters of \pft{} model but $c_2$ and $c_4$ to the same values as in previous section. The remaining free parameters were varied at equal increments of $0.1$ within the ranges
...
We then computed the sum-of-squares distances between each of the simulated trajectories and their \gls{gssa} equivalents and summarised the data into a set of contour plots, such as the ones in \autoref{fig:hit-and-miss:ode15s}, which we discuss bellow.

We can immediately see from the \autoref{fig:hit-and-miss:ode15s:1} that the distance landscape is anything but uniform. There seem to be areas of the parameter space where the system is easier to approximate than in other areas. In these areas, even the first-order moments are sufficient to capture the overall behaviour of the system accurately. \autoref{fig:hit-and-miss-interesting-points:ode15s:1:bullet} plots a set of first-order trajectories for one of the easy-to-approximate parameters (indicated by $\bullet$ in the contour point). 
In the said figure we can observe the typical damped oscillatory behaviour of the \gls{gssa} trajectory, that results from out-of-frequency oscillations of individual molecule concentrations in the \pft{} system. 
This observation suggests that it was not the dynamics of the actual system that make it easier to approximate the behaviour for these parameters, but something intrinsic to the approximation method.

Conversely, we can also see regions of the parameter space where the approximation seems to perform poorly, i.e. the bottom-right corner of the \autoref{fig:hit-and-miss:ode15s:1}. This already hard-to-approximate area seems to have mountains of even higher to approximate values, such as the one indicated by an $\times$. The trajectories at this last parameter set together with the trajectories for the parameter set marked with a $\blacksquare$ are shown in \autoref{fig:hit-and-miss-interesting-points:ode15s:1}. 
We can see that the oscillations for the $\blacksquare$ parameters are increasing and not reducing in amplitude, going into the invalid negative values for the concentrations along the way. 
The simulated trajectory for the $\times$, on the other hand, just plummets into the large negative values and does not recover.
Even when forgetting the fact that the concentrations obtained are not realistic for a second, the trajectories generated do not match the true behaviour of the system, and therefore both of these trajectories are not acceptable approximations of the true thing.


\begin{figure}
    \begin{subfigure}[t]{0.31\textwidth}
        \includegraphics[scale=0.4]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-1-scalar-True-p53-90.0_0.002_1.6_1.1_2.1_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters indicated by $\bullet$}
        \label{fig:hit-and-miss-interesting-points:ode15s:1:bullet}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.31\textwidth}
        \includegraphics[scale=0.4]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-1-scalar-True-p53-90.0_0.002_2.4_1.1_0.7_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters ($\blacksquare$)}
        \label{fig:hit-and-miss-interesting-points:ode15s:1:blacksquare}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.31\textwidth}
        \includegraphics[scale=0.4]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-1-scalar-True-p53-90.0_0.002_2.5_1.1_1.8_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters ($\times$)}
        \label{fig:hit-and-miss-interesting-points:ode15s:1:bullet}
    \end{subfigure}
    \caption{\emph{The \gls{mea} approximated and \gls{gssa} simulated trajectories for the indicator points in the \autoref{fig:hit-and-miss:ode15s:1}}. Only one of the three model species is shown. The distance measure in the bottom left corner shows the distance only for the species in the plot. Note the different $y$ axis scale for the plots.}
    \label{fig:hit-and-miss-interesting-points:ode15s:1}     
\end{figure}

Moving onto the next subplot of the contour figure, the subplot where maximum moment expansion order is set to two, \autoref{fig:hit-and-miss:ode15s:2}, we can see that the high-distance area contour seems to shrink, compared to the previous subplot, putting the $\times$ and $\blacksquare$ parameters into a lower-distance zone. The resulting trajectories for these two parameter sets are shown in \autoref{fig:hit-and-miss-interesting-points:ode15s:2}. 
The trajectory for the parameter set $\blacksquare$ is particularly interesting (\autoref{fig:hit-and-miss-interesting-points:ode15s:2:blacksquare}) as it seems to show the oscillations that increase, and then decrease in amplitude: a behaviour that is not even close to the actual behaviour seen from the \gls{gssa} simulations, but nevertheless unusual.
The behaviour for the other parameter set shows damped oscillations, however, they are not nearly damped enough to be able to match the results from the stochastic simulations.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{\emph{The \gls{mea} approximated and \gls{gssa} simulated trajectories for the indicator points in the \autoref{fig:hit-and-miss:ode15s:2}}. Only one of the three model species is shown. The distance measure in the bottom left corner shows the distance only for the species in the plot.}
    \label{fig:hit-and-miss-interesting-points:ode15s:2}     
\end{figure}

Moving over to the distance landscape generated by the third order approximation, we start to see our first share of parameters that \texttt{ode15s} finds hard to produce any output at all (\autoref{fig:hit-and-miss:ode15s}). 
In order to test if there was a problem with \texttt{ode15s} solver in the particular, we have attempted other ODE solvers with similar results (data not shown)\sauliustodo{This is individual-report material -- let's make it a "supplementary figures" alternative}.

We can see the that the parameter $\times$ gets into a lower distance tier in when the maximum order is set to 4 (\autoref{fig:hit-and-miss:ode15s:4}), and moves back to the border between the two tiers when the maximum order is set to 6 (\autoref{fig:hit-and-miss:ode15s:6}).

\sauliustodo[inline]{See if we can finish generating max order=8 figures overnight. Add trajectory figures for $\bullet$, $\times$ and $\ast$ at the highest order we could generate. Mention that $\bullet$ stays exactly the same regardless of the order, but $\times$ increases progressively till order 4 and then starts to diverge, while $\ast$ seems to just keep getting better and better.}

\sauliustodo[inline]{Show the distance plots for log-normal, talk a wee bit about them, and I think that's us done with this section}

%\sisitodo[inline]{In your section, when we say surprising result that the trajectories are nearly perfect with low orders, you can change it to something like "surprising, but consistent with the results in \autoref{sec:hit-and-miss}"}
...
we generated $5000$ \acrfull{gssa} simulations of the \pft{} with a certain parameter set. We believe this number of \gls{gssa} simulations to be sufficient to capture the behaviour of this simple system in full.
...
Surprisingly, when parameters are free, the inference procedure was able to find a set of parameters, but very different from the correct ones, for which observed and theoretical trajectories are extremely similar (see \autoref{fig:range-free-parameter-inference}).
...
\begin{figure}
    ...
    \caption{\emph{Incorrectly inferred parameter values can generate trajectories matching the ``experimental'' data at maximal order $1$.}
Each column represents one species in the \pft{} model. 
The starting trajectories were generated based on the true parameter values, 
as the inference procedure started from the true values, 
while the optimal trajectories were generated based on the inferred parameters, where all seven parameters were free.
Averaged trajectories from $5000$ \gls{gssa} simulations were regarded as experimental data to compare with the starting and optimal trajectories.}
...
To do this, we limited our parameter inference procedure to have only a pair of free parameters at a time (all other parameters are fixed to their true values).
We performed this for all combinations of two parameters and checked if we can reproduce the interesting behaviour.
...
\begin{figure}
    ...
    \label{fig:inference_phenomenon:maxorder:1:trajectories}
    \end{subfigure} 
    \caption{\emph{Contour plot of distance to \gls{gssa} trajectory and trajectories for parameter inference performed with lower order \gls{mea} using \pft{} model show incorrect inference.}
    ...
    Each row displays a contour plot and correspondent trajectories using different maximal order in \gls{mea}.
    In the contour plots, the warmer the colour, the more distant the inferred trajectories are from the \gls{gssa} trajectories, and the numbers indicate the distance between optimal trajectories obtained from inference and the \gls{gssa} trajectories.
    In the trajectory plots, the distance between the optimal and \gls{gssa} trajectories are indicated at the bottom. 
    Although the optimal trajectories match with the \gls{gssa} trajectories, the correspondent contour plot shows wrong values are inferred.
...
Through further investigation in the contour plots and trajectories with different maximal orders, we found that inference based on lower order (especially maximal order 1) \gls{mea} is more problematic than inference based on higher order \gls{mea}. 
This might because based on lower order \gls{mea}, the system is able to explore a larger region and identify a low-distance region with parameters that are easier to approximate with, and leads to incorrect results.
For example, optimal trajectories simulated at lower order can match with the \gls{gssa} trajectories completely, which is also consistent with the result in \autoref{sec:hit-and-miss}. 
Moreover, in \autoref{fig:inference_phenomenon:maxorder:1:landscape}, the starting point of parameter inference represents the true values, but is labelled as a high-distance zone, 
because \gls{mea} cannot capture the damped oscillatory behaviour of the system for this set of parameters without higher order moments, as illustrated by the starting trajectories in \autoref{fig:inference_phenomenon:maxorder:1:trajectories}. 
...
Generally, inference based on higher order \gls{mea} is able to produce more realistic results, as reflected by less distance between the starting trajectories and the \gls{gssa} trajectories, and shallower distance gradients in the contour plots. The slightly more correct inference may be a result of more accurate \gls{mea} approximation. However, the parameter inference process is still not stationary and is able to drift away from the correct set of parameters. The complete set of observations is provided in \autoref{fig:parameter_inference_landscape_trajectories}.
...
\caption{\emph{Contour plot of distance to \gls{gssa} trajectory and trajectories for parameter inference performed with higher order \gls{mea} using \pft{} model.}  
    Among seven parameters, only the values for $c_2$ and $c_6$ are inferred. The starting values were obtained from previous inference using the true parameter values.
    All the contour plots and the trajectories start with the same parameter values for inference.
    Each row displays a contour plot and correspondent trajectories using different maximal order in \gls{mea}.
    In the contour plots, the warmer the colour, the more distant the inferred trajectories are from the \gls{gssa} trajectories, and the numbers indicate the distance between optimal trajectories obtained from inference and the \gls{gssa} trajectories.
...
In conclusion, parameter inference does not work well in general. Recovering the correct parameter set remains challenging, even with higher order \gls{mea} approximations. This observation raises concerns about the validity of parameter inference approaches using \gls{mea} approximation. 
