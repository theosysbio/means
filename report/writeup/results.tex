...
In order to minimise the overhead of computations, we use the {\tt autowrap} module, built into {\tt sympy} to compile our numeric expressions into C and then wrap the same \py{} interface around them so the end user does not see any difference. 
This process adds some overhead, of course, as the system needs to be compiled from scratch. 
We have managed to control this overhead by caching these precompiled expressions in memory. 
Due to this caching, this overhead is present only in the first evaluation of expressions, while all other evaluations are an order of magnitude faster. 
This performance improvement is illustrated further in the tutorial \autoref{sec:reuse_of_simulation_objects}.

\subsubsection{Comparing the Performance of Different \gls{ode} Solvers}
\todo[inline]{New Section: Please review}
...
Choice of the \emph{best} solver for one's \acrlongpl{ode} system depends very much on the particular problem one is trying to simulate, however certain solvers tend to be accepted more generally than others.

CVODE\citationneeded{CVODE?} is an example of one of the widely used solvers. As described in \autoref{sec:ode_simulations}, we implement two variants of this solver, \verb#ode15s# and \verb#cvode#.
Each of these solvers point to the same back-end implementation of CVODE provided by \verb#assimulo# package\citationneeded{assimulo}. The \verb#ode15s# solver just comes with a set of parameters that mimics the \mat{} solver with the same name.
...
The complete list of the solvers used in available in the documentation\citationneeded{refer to docs}. 
...
Here we base our comparison on the runtimes of the solvers, in the later \sauliustodo{ref to new section once I create it} we compare them in terms of accuracy and stability as well.
...
The set of parameters obtained by inference were very different from the actual \gls{gssa} simulations, but the approximated trajectories were identical (see \autoref{fig:parameter_pairs}. This strongly questions the suitability of parameter inference procedures in obtaining the correct parameter sets.
