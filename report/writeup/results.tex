...
\begin{figure}
    ...
    \caption{\emph{Contour plots of the sum-of-squares distance between the \gls{mea} approximated trajectory and the \gls{gssa} simulated trajectory for scalar closure and \texttt{ode15s} solver}.
    The grid of distances was calculated by sampling each of the two parameters $c_2$ and $c_4$ at $0.1$ intervals and comparing the simulated trajectories to the \gls{gssa} trajectory.
    ...
    It also appears that even maximal moment orders are more stable than the odd ones.}
...
These two parameters were varied by $0.1$ increments of within the ranges $c_2 \in [1.5, 2.7)$ and $c_4 \in [0.6, 2.7)$ (where previously $c_2=1.7$ and $c_4=0.93$).
For ever every combination of these parameters, the average of \gls{gssa} $5000$ trajectories was generated as ground truth data.
...
The distance landscape was not uniform. For instance, distance to \gls{gssa} could range from $100$ to $10^9$ (\autoref{fig:hit-and-miss:ode15s:1}).
...
\autoref{fig:hit-and-miss-interesting-points:ode15s:1:bullet} plots a set of first-order trajectories for one of the easy-to-approximate parameters (indicated by $\bullet$ in the contour point).
We observe the typical damped oscillatory behaviour of the \gls{gssa} trajectory,
which results from individual trajectories stochasticaly getting out-of-phase\cite{ale_general_2013}.
This observation suggests that it was not the dynamics of the actual system that make it easier to approximate the behaviour for these parameters,
but something intrinsic to the approximation method. \sauliustodo{sentence unclear and convoluted (for me). split to two sentences?}
...
This already hard-to-approximate area seems to have mountains of even higher to approximate values, such as the one indicated by a $\times$ symbol.
...
We can see that the oscillations for the $\blacksquare$ parameters are increasing, and not reducing, in amplitude.
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.31\textwidth}
        ...
        \label{fig:hit-and-miss-interesting-points:ode15s:1:bullet}
    \end{subfigure}
    \caption{\emph{The \gls{mea} approximated and \gls{gssa} simulated trajectories for the indicator points in the \autoref{fig:hit-and-miss:ode15s:1}}.
...
%This behaviour that is not even close to the actual behaviour seen from the \gls{gssa} simulations, but nevertheless unusual.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{\emph{The \gls{mea} approximated and \gls{gssa} simulated trajectories for the indicator points in the \autoref{fig:hit-and-miss:ode15s:2}}.
    Only one of the three model species is shown.
    The distance measure in the bottom left corner is only for the species in the plot.}
    \label{fig:hit-and-miss-interesting-points:ode15s:2}     
\end{figure}

For third order approximation (\autoref{fig:hit-and-miss:ode15s:3}), the solver (\texttt{ode15s}) fail to simulate trajectories for a parameter set ($\blacksquare$).
Using alternative \gls{ode} solvers did not allow to simulate these trajectories (data not shown)\sauliustodo{This is individual-report material -- let's make it a "supplementary figures" alternative}.

We can see the that the parameter $\times$ gets into a lower distance tier in when the maximum order is set to 4 (\autoref{fig:hit-and-miss:ode15s:4}),
and moves back to the border between the two tiers when the maximum order is set to 6 (\autoref{fig:hit-and-miss:ode15s:6}).
