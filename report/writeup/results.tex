...
\subsection{Point estimates from resulting inference procedures raise concerns}
...
We first used the \pft{} model expressed only in terms of first, third and fifth order moments\sisitodo[inline]{Misleading: we performed three inferences with three different max-orders, your sentence leads to believe we performed one with max ord=5, but removed equations for 2nd and fourth orders}, as approximation based on first order may be inaccurate for the particular system, and higher order moments might be necessary to capture the damped oscillations present in the means of \gls{ssa} simulations\cite{ale_general_2013}. 
...
Surprisingly, when all parameters were free to be explored, the inference procedure was able to find a set of parameters that are very different from the correct ones, for which observed and theoretical trajectories are extremely similar even for maximal expansion order 1 (see \autoref{fig:range-free-parameter-inference-max-order-1} for result based on \gls{maxord}). 
The observation is consistent with inference using  \gls{maxord} of three (data not shown). \sisitodo[inline]{I do not see any reason to talk about the inference of order three and five for 7 free parameters for just the sake of mentioning it. We do not show any data for it, and it only adds confusion for the reader. 
Remove mentions for free inference of orders 3-5 from group report (including the mention one todo above), and add it to your individual one if you feel it relevant.}
This result was \sisitodo{expected? for maximum order=1? am I missing smth here?}expected, because, as mentioned previously, higher order moments are believed to be necessary to capture the system behaviour.
...
In order to gain some insight into these \sisitodo{now it is unexpected, make up your mind :) }unexpected inference results, we first attempted to restrict ourselves within a two-dimensional space, which is easier to work with.
