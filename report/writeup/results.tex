...
In a first place, the calls to \texttt{sympy.simplify()} where \quentintodo{do you mean "were removed"?} removed (b). 
Then, \texttt{sympy.xreplace()} was used instead of \texttt{sympy.substitute()} (c). 
Generating an $(n-s) \times (n_2-s + 1)$ \quentintodo{what do you mean by $n_2$? } matrix (d), as opposed to an $(n-s) \times (n-s + 1)$ one, also increases speed.
...
The time complexity appears exponential ($O(2^n)$, \quentintodo{might be a stupid question, but what's $O$?} where $n$ is the maximal moments order) in every case, 
