...
Through further investigation in the contour plots and trajectories with different maximal orders, we found that inference based on lower order (especially first order) \gls{mea} is more problematic than inference based on higher order \gls{mea}. 
For example, in \autoref{fig:inference_phenomenon:maxorder:1:landscape}, the starting point of parameter inference represents the true values, but is labelled as a high-distance zone. 
This is because \gls{mea} cannot capture the damped oscillatory behaviour of the system for this set of parameters without higher order moments \cite{probably need a ref here}, as illustrated by the starting trajectories in \autoref{fig:inference_phenomenon:maxorder:1:trajectories}. 
Surprisingly, based on lower order \gls{mea}, the system is able to explore a larger region and identify a low-distance region that is far away from the expected point.
...
Inference based on higher order \gls{mea} is able to produce slightly more realistic result. As the maximal order increases, the starting trajectories is getting closer to the \gls{gssa} trajectories, and the distance gradients in the contour plots become more and more level for higher order moments. However, the parameter inference process is still not stationary and is able to  drift away from the correct set of parameters. The complete set of observations is provided in \autoref{fig:parameter_inference_landscape_trajectories}.
