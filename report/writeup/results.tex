...
\caption{\emph{Distance landscape and trajectories for each species using different maximal orders.} Among seven parameters, only the values for $c_2$ and $c_6$ are inferred, with starting values inferred from inference using the true values. In the distance landscape on the left, the warmer the colour, the further the inferred trajectories are from the \gls{gssa} trajectories. The \gls{gssa} trajectories are generated using the new parameter values labelled as \emph{start}. In the trajectories for each species, the starting trajectories are simulated using the starting values of $c_2$ and $c_6$, the optimal trajectories are generated based on the end point in distance landscape, and the \gls{gssa} trajectories are also generated based on the end point in the distance landscape.} 
\label{fig:parameter_inference_c2_c6}
\end{figure}
