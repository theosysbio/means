...
\begin{figure}
   ...
   \caption{\emph{The trajectories simulated for the parameter set marked by $\bullet$ symbol in \autoref{fig:hit-and-miss:ode15s} stay relatively similar for all values of max order parameter we have tested.} Only one of the three species in the model is shown. The distance measure in the bottom right corner shows the distance computed for only these species.
...
As seen in the \autoref{fig:hit-and-miss:ode15s}, solvers tend to perform a bit better when the moment expansion is closed at even numbers. We can see the that the parameter $\times$ gets into a lower distance tier in when the maximum order is set to 4 (\autoref{fig:hit-and-miss:ode15s:4}),
and moves back to the border between the two tiers when the maximum order is set to 6 (\autoref{fig:hit-and-miss:ode15s:6}). Similarly, the approximations at the parameter set marked with an $\ast$ become better
and better as the low-distance area grows. 
