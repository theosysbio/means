...
we generated $5000$ \acrfull{gssa} simulations of the \pft{} with a certain parameter set. 
We believe this number of \gls{gssa} simulations is sufficient to capture the behaviour of this simple system in full.
...
This approximation was bound to be very inaccurate for the particular system, as higher order moments are necessary to capture the damped oscillations present in the means of \gls{gssa} simulations\cite{ale_general_2013}.
...
\begin{figure}
    ...
    \caption{\emph{Incorrectly inferred parameter values can generate trajectories matching the empirical data at maximal order of $1$.}
...
Averaged trajectories from $5000$ \gls{gssa} simulations ($\times$) were regarded as experimental data to compare with the starting and optimal trajectories.}
...
\begin{figure}a region large enough
    \centering
    ...
    \label{fig:inference_phenomenon:maxorder:1:trajectories}
    \end{subfigure} 
    \caption{\emph{Contour plots of distance to \gls{gssa} trajectory and trajectories for parameter inference performed with lower order \gls{mea} using \pft{} model showing incorrect inference.}
    ...
    Each row displays a contour plot and corresponding trajectories using different maximal order in \gls{mea}.
    Note that the scales in the contour plots using different maximal orders are different.
    ...
    Although the optimal trajectories (blue) match with the \gls{gssa} trajectories ($\times$), the corresponding contour plot shows wrong values are inferred.
    The white space in the contour plots represent the unexplored parameter space.}
    \label{fig:inference_phenomenon:lower_order}
\end{figure}


Through further investigations in the contour plots and trajectories with different maximal orders, 
...
For example, optimal trajectories simulated at lower order can mabut not necessarily the true parameter valuestch with the \gls{gssa} trajectories completely, which is also consistent with the result in \autoref{sec:hit-and-miss}. 
...
\caption{\emph{Contour plots of distance to \gls{gssa} trajectory and trajectories for parameter inference performed with higher order \gls{mea} using \pft{} model.}  
    ...
    In the contour plots, the warmer the colour, the more distant the inferred trajectories are from \gls{gssa} trajectories, and the numbers indicate the distance between optimal trajectories and \gls{gssa} trajectories.
    The white space in the contour plots represent the parameter space not explored by the system. 
    In the trajectory plots, the distance between the optimal (blue) and \gls{gssa} trajectories ($\times$) are indicated at the bottom.}
