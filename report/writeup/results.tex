...
\subsubsection{Optimisation of Symbolic Computations}
...
Results are summarised by sets of contour plots (Figures~\ref{fig:hit-and-miss:ode15s}~and~\ref{fig:hit-and-miss:ode15s:log-normal}) which we discuss bellow.
...
\subsubsection{Accuracy of Scalar Closure}
...
\begin{figure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[scale=0.55]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-1-scalar-True-p53-90.0_0.002_2.4_1.1_0.7_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters ($\blacksquare$)}
        \label{fig:hit-and-miss-interesting-points:ode15s:1:blacksquare}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[scale=0.55]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-1-scalar-True-p53-90.0_0.002_2.5_1.1_1.8_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
...
\begin{figure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[scale=0.55]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-2-scalar-True-p53-90.0_0.002_2.4_1.1_0.7_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters indicated by $\blacksquare$}
        \label{fig:hit-and-miss-interesting-points:ode15s:2:blacksquare}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[scale=0.55]{{../pipeline/task-output/FigureSSAvMEATrajectory/FigureSSAvMEATrajectory-pdf-2-scalar-True-p53-90.0_0.002_2.5_1.1_1.8_0.96_0.01-70.0_30.0_60.0-0.0_40.0_0.1-ode15s--5000}.pdf}
        \caption{Parameters ($\times$)}
...
%as approximation based on first order may be inaccurate for the particular system, and higher order moments might be necessary to capture the damped oscillations present in the means of \gls{ssa} simulations\cite{ale_general_2013}. 

We first used the \pft{} model expressed only in terms of first order moments, 
and deliberately started the inference from the correct parameter values, expecting an immediate convergence (\ie{} no movement in the parameter space).
...
Surprisingly, when all parameters were free to be explored, the inference procedure was able to find a set of parameters that are very different from the correct ones, for which observed and theoretical trajectories are extremely similar even for maximal expansion order 1 (see \autoref{fig:range-free-parameter-inference-max-order-1}). 
%The observation is consistent with inference using  \gls{maxord} of three (data not shown). 
%This result was unexpected because, as mentioned previously, higher order moments are believed to be necessary to capture the system behaviour.

\begin{figure}
    \centering
    \includegraphics[width=0.95\textwidth]{{../pipeline/task-output/FigureInferenceStartEndSSA/Range-free-parameter-inference}.pdf}
    \caption{\emph{Incorrectly inferred parameter values can generate trajectories matching the empirical data at \gls{maxord} of one.}
...
We performed these experiments for all 21 combinations of two parameter pairs.
...
\begin{figure}[tb]
    ...
    Trajectories are simulated using \gls{maxord} of one.}
    \label{fig:parameter_pairs}
\end{figure}

Interestingly, we were able to observe the same behaviour for the parameter pairs $(c_2, c_6)$, $(c_0, c_1)$, $(c_1, c_6)$, and, finally, $(c_0, c_6)$ (first two parameter pairs are pictured in the \autoref{fig:parameter_pairs}).
We then chose the pair of parameters that allowed the inference procedure to converge to a trajectory with minimal distance to the \gls{ssa} average: $c_2$ and $c_6$.
...
\begin{figure}
    ...
    \caption{}
    \label{fig:inference_phenomenon:maxorder:2:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.7\textwidth}
    ...
    \caption{}
    \label{fig:inference_phenomenon:maxorder:2:trajectories}
    \end{subfigure} 
    ...
    All the contour plots (subplots a and c) and the trajectories (subplots b and d) start with the same parameter values for inference.
...
\begin{figure}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:3:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.7\textwidth}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:3:trajectories}
    \end{subfigure}   
     ~  
    \begin{subfigure}[t]{0.25\textwidth}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:4:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.7\textwidth}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:4:trajectories}
    \end{subfigure}   
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:5:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.7\textwidth}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:5:trajectories}
    \end{subfigure}
    
    \caption{Figures continued on the next page}
    \label{}
\end{figure}
...
\begin{figure}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:6:landscape}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.7\textwidth}
    ...
    \caption{}
    \label{fig:incerence_phenomenon:maxorder:6:trajectories}
    \end{subfigure}
...
Through further investigations in the contour plots and trajectories with different \glspl{maxord}, 
...
Fortunately, these experiments still provide some light for the method.
