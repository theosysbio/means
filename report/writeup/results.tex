...
\autoref{fig:hit-and-miss-interesting-points:ode15s:1:bullet} plots the first-order trajectories associated with the one of the easy-to-approximate parameters (indicated by $\bullet$ in the contour point).
...
Because of this, we could guess there is something intrinsic to the approximation method, rather than to the system itself, that causes this phenomenon.\quentintodo{Any better now?}
...
We can see that the oscillations for the $\blacksquare$ parameters are increasing, and not reducing, in amplitude -- completely opposite to what we observe in the \gls{gssa} data.
...
Alternative \gls{ode} solvers did also struggle to simulate these trajectories and did not produce any results (data not shown)\sauliustodo{This is individual-report material -- let's make it a "supplementary figures" alternative}.
