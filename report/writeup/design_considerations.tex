...
The approximation submodule implements \gls{lna} and \gls{mea} methods for biological systems encoded by the previously mentioned \verb"Model" objects (see \autoref{sec:Model-definition} \sauliustodo{not sure how to fix this} for a detailed description on how this encoding is done).
...
The advantages described above apply mostly to software using all or part of the \means{} package to perform designated tasks\sauliustodo{QG: not v clear}.
...
In order to overcome this, we also provide shorthand functions \verb"mea_approximation" and \verb"lna_approximation", the use of which is demonstrated in \autoref{sec:approximation-example} \sauliustodo{autoref problem}.
...
Examples on how to use both classes are provided in \autoref{sec:example-simulation}\sauliustodo{another autoref}.
