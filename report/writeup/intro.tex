...
This has been shown to result in quantitative inaccuracy\citationneeded{Either add a citation here, or change "has been shown to" to "can"}, and, in the worst cases, qualitatively erroneous predictions.
...
This method is valid when the amount of each species is large and systems consist of first order reactions
(such as  $a \rightarrow b$, but not $a + c  \rightarrow b$)\todo{I thought non-linearity was the problem, not having two things on LHS -- S}.
...
This makes the expressions for moments infinitely long, and the \glspl{ode} impossible to solve \todo{what about adding (``or even write down")} analytically. In order to make the approach computable, a strategy for stopping the expansion of moment terms by approximating (closing) higher order moments is necessary.
