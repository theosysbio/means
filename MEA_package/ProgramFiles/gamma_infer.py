...
from math import sqrt, floor, gamma, exp, log, pi, lgamma, factorial
...
#####################################################################
# Get indices for mean/variance data for each species in CVODE output
# Returns as lists mean_id and var_id where position in list corresponds
# to species number.  Also returns the list sp_id, this contains the
# species number for each of the experimental data sets/trajectories.
#####################################################################
...
def read_maxent_output():
    maxent_out = open('maxent_out.txt')
...
def optimise(param, vary, initcond, varyic, limits,sample, cfile, mfkoutput, distribution):
    ...
    result = fmin(distance, i0, args=(param, vary,initcond, varyic, sp_id,mu, t, cfile, mean_id, var_id, distribution), ftol = 0.000001,disp=0, full_output=True,callback=my_callback)
