...
from means.core import Moment, VarianceTerms, ODEProblem
        ...
        for i in range(len(ymat)):
            ...
            for j in range(len(ymat)):
                if i <= j:
                    ...
                    variance_terms.append(VarianceTerms(position=(i,j), symbol=symbol))
                else:
                    # Since Vi,j = Vj,i, i.e. covariance are equal, we only record Vi,j but not Vj,i
                    symbol = 'V_{0}_{1}'.format(j, i)
                    variance_terms.append(VarianceTerms(position=(j,i), symbol=symbol))
                row.append(symbol)
